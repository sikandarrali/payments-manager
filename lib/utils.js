...
export const getDayFromFormattedDate = (date) => {
	return date.split("-")[0]
}

export function getOrdinalDay(day) {
	const suffix = ["th", "st", "nd", "rd"];
	const numericDay = Number(day); // Remove leading zero if any
	const value = numericDay % 100;

	return (
		<span>
			{numericDay}
			<sup>{suffix[(value - 20) % 10] || suffix[value] || suffix[0]}</sup>
		</span>
	);
}
